...
class Sendmail extends AppController {
    ...
    function main() {
        ...
        if (isset($_POST['data'])) {
            ...
            if (App::db()->result() !== false)
                exit(json_encode(['ok' => 'exist']));

            //Gravando na tabela de usuários temporários (até o cadastro)
            $key = (new Lib\Can)->encode((microtime(true) * 100000));
            App::db()->query('INSERT INTO user_tmp (TOKEN,EMAIL,IDATE)
                                          VALUES (:tk,:mail,:idate)', [':tk' => $key, ':mail' => $mail, ':idate' => date('Y-m-d H:I:s')]);
            //Enviando EMAIL
