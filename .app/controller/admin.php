...
class Admin extends Qcontroller {
    ...
    function main()
    {
        $key = file_get_contents(ROOT.'.app/config/public.key');
    ...
    function saveKeys(){
        ...
        if(isset($rec['dec']->public)) file_put_contents(ROOT.'.app/config/public.key', $rec['dec']->public);
        if(isset($rec['dec']->private)) file_put_contents(ROOT.'.app/config/private.key', $rec['dec']->private);
