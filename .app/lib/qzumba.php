...
class Qzumba {
     ...
     *
     */
    static function respond($accType = 'all', $reg = '', $ctrl='Default', $method='main'){
        $accType = strtoupper(trim($accType));
        if($accType == 'ALL' || !is_string($accType)) $accType = 'GET|POST|PUT|DELETE|OPTIONS|PATCH|HEAD';
        $mt = explode('|', $accType);
        foreach($mt as $mtd){

            static::$config['routers'][$mtd][]=['rqst'=>$reg, 'ctrl'=>$ctrl, 'method'=>$method];
        }
    }
